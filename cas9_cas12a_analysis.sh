...
WORKING_DIR=/Users/jlevendis/cas12_vs_cas9_pam_analysis
OFFSET_DISTANCE=50
OUTPUT_DIR=${WORKING_DIR}/pam_analysis
mkdir -p ${OUTPUT_DIR}

# using rqc commit d0ca7f0b4b06b77351d859c3bd2cdbdd195cb34d
RQC_PATH=/Users/jlevendis/crypto_cas12_analysis/rqc/rqc.py
...
python ${RQC_PATH} calculate_offsets -d ${OFFSET_DISTANCE} -r ${OUTPUT_DIR}/${PROJECT_PREFIX}_start_codons.tsv -o ${OUTPUT_DIR}/${PROJECT_PREFIX}_start_codon_offsets_${OFFSET_DISTANCE}.tsv -i NGG ${OUTPUT_DIR}/${PROJECT_PREFIX}_NGG_all.tsv TTTN ${OUTPUT_DIR}/${PROJECT_PREFIX}_TTTN_all.tsv
python ${RQC_PATH} calculate_offsets -d ${OFFSET_DISTANCE} -r ${OUTPUT_DIR}/${PROJECT_PREFIX}_stop_codons.tsv -o ${OUTPUT_DIR}/${PROJECT_PREFIX}_stop_codon_offsets_${OFFSET_DISTANCE}.tsv -i NGG ${OUTPUT_DIR}/${PROJECT_PREFIX}_NGG_all.tsv TTTN ${OUTPUT_DIR}/${PROJECT_PREFIX}_TTTN_all.tsv

# plot offsets
python ${RQC_PATH} plot_relative_distance -l "start codon" -d ${OFFSET_DISTANCE} -i ${OUTPUT_DIR}/${PROJECT_PREFIX}_start_codon_offsets_${OFFSET_DISTANCE}.tsv -o ${OUTPUT_DIR}/${PROJECT_PREFIX}_${OFFSET_DISTANCE}_start.eps
python ${RQC_PATH} plot_relative_distance -l "stop codon" -d ${OFFSET_DISTANCE} -i ${OUTPUT_DIR}/${PROJECT_PREFIX}_stop_codon_offsets_${OFFSET_DISTANCE}.tsv -o ${OUTPUT_DIR}/${PROJECT_PREFIX}_${OFFSET_DISTANCE}_stop.eps
...
printf "average NGG count around start codons: "; awk -F'\t' 'NR>1 {sum += $4; count++} END { if (NR > 0) print sum / count}' ${OUTPUT_DIR}/${PROJECT_PREFIX}_start_codon_offsets_${OFFSET_DISTANCE}.tsv
printf "average TTTN count around start codons: "; awk -F'\t' 'NR>1 {sum += $6; count++} END { if (NR > 0) print sum / count}' ${OUTPUT_DIR}/${PROJECT_PREFIX}_start_codon_offsets_${OFFSET_DISTANCE}.tsv
printf "average NGG count around stop codons: "; awk -F'\t' 'NR>1 {sum += $4; count++} END { if (NR > 0) print sum / count}' ${OUTPUT_DIR}/${PROJECT_PREFIX}_stop_codon_offsets_${OFFSET_DISTANCE}.tsv
printf "average TTTN count around stop codons: "; awk -F'\t' 'NR>1 {sum += $6; count++} END { if (NR > 0) print sum / count}' ${OUTPUT_DIR}/${PROJECT_PREFIX}_stop_codon_offsets_${OFFSET_DISTANCE}.tsv